...
The second line adds PyMuPDF, which reads the PDF; without it the tool still
checks a `.txt` file. The address needs no credential: the repository is
public.

What I can also run is the same
